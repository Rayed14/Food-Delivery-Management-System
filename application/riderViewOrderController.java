...
public class riderViewOrderController implements Initializable {
    ...
    @FXML
    void selectOrdertoComplete(ActionEvent event) throws IOException {
    	...
    	if (OrdersTable.getSelectionModel().getSelectedIndex() == -1)
    	{
    		...
    		alertObj.setContentText("Please select an Order before pressing the complete Order button.");
    	...
    	else
    	{
			...
			if (!returnObj.getOrderStatus().equals("delivering"))
			{
				Alert alertObj = new Alert(Alert.AlertType.WARNING);
	    		alertObj.setHeaderText("Correct Order not Selected");
	    		alertObj.setTitle("Warning");
	    		alertObj.setContentText("Please select a delivering Order.");
	    		alertObj.showAndWait();
			}
			else
			{
				...
				contr.competeOrderRider(orderObj);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		ArrayList<Order> orderList = contr.getRiderOrders();
